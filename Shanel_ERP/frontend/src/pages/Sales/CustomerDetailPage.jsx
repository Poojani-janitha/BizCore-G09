        ...
        return (
            <div className="d-flex align-items-center justify-content-center" style={{ minHeight: '60vh' }}>
                <div className="text-center">
                    ...
                    <p className="text-muted small fw-bold text-uppercase">Loading customer...</p>
                </div>
            </div>
        );
    }
    ...
    if (!data) {
        return (
            <div className="container-fluid p-4">
                <div className="alert alert-danger rounded-4 border-0">Customer not found.</div>
            </div>
        );
    }
            ...
            <div className="row g-4">
                ...
                <div className="col-lg-4">
                    ...
                    <div className="card border-0 shadow-sm rounded-4 p-4 mb-3"
                        ...
                        <p className="small fw-bold text-uppercase mb-1 opacity-75" style={{ fontSize: '10px', letterSpacing: '0.5px' }}>Outstanding Balance</p>
                        ...
                        {customer.Credit_Allowed && (
                            <div>
                                ...
                                <small className="opacity-75 mt-1 d-block">Limit: Rs.{parseFloat(customer.Credit_Limit).toLocaleString()}</small>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="row g-2 mb-3">
                        <div className="col-6">
                            <div className="card border-0 shadow-sm rounded-4 p-3 bg-white text-center">
                                ...
                                <p className="text-muted small fw-bold text-uppercase mb-0" style={{ fontSize: '9px' }}>Total Purchases</p>
                        ...
                        <div className="col-6">
                            <div className="card border-0 shadow-sm rounded-4 p-3 bg-white text-center">
                                ...
                                <p className="text-muted small fw-bold text-uppercase mb-0" style={{ fontSize: '9px' }}>Invoices</p>
                    ...
                    <div className="card border-0 shadow-sm rounded-4 p-4 bg-white">
                        ...
                        <InfoRow icon={<Phone size={14} />} label="Phone 1" value={customer.Phone1} />
                        <InfoRow icon={<Phone size={14} />} label="Phone 2" value={customer.Phone2} />
                        <InfoRow icon={<Mail size={14} />} label="Email" value={customer.Email} />
                        <InfoRow icon={<MapPin size={14} />} label="Address" value={[customer.Address, customer.City].filter(Boolean).join(', ')} />
                        <InfoRow icon={<User size={14} />} label="Contact Person" value={customer.Contact_Person} />
                        <InfoRow icon={<CreditCard size={14} />} label="Payment Terms" value={customer.Payment_Terms} />
                        <InfoRow icon={<CheckCircle size={14} />} label="Last Purchase" value={customer.Last_Purchase_Date ? new Date(customer.Last_Purchase_Date).toLocaleDateString() : null} />
                    </div>
                </div>
                ...
                <div className="col-lg-8">
                    ...
                    <div className="d-flex gap-2 mb-3">
                        {[
                            { key: 'overview', label: 'Purchase History', icon: <ShoppingBag size={14} /> },
                            { key: 'statement', label: 'Full Statement', icon: <FileText size={14} /> }
                        ].map(tab => (
                            <button
                                key={tab.key}
                    ...
                    {activeTab === 'overview' && (
                        <div className="card border-0 shadow-sm rounded-4 overflow-hidden bg-white">
                            <div className="card-header bg-white border-0 py-3 px-4">
                                <h6 className="fw-bold text-dark mb-0">Recent Invoices</h6>
                            </div>
                            <div className="table-responsive">
                                <table className="table table-hover align-middle mb-0">
                                    <thead className="bg-light">
                                        <tr>
                                            {['Invoice #', 'Date', 'Type', 'Total', 'Paid', 'Balance', 'Status'].map(h => (
                                    ...
                                    <tbody>
                                        {recentSales.length === 0 ? (
                                            <tr><td colSpan={7} className="text-center py-5 text-muted small">No invoices found</td></tr>
                    ...
                    {activeTab === 'statement' && (
                        <div className="card border-0 shadow-sm rounded-4 overflow-hidden bg-white">
                            <div className="card-header bg-white border-0 py-3 px-4">
                                <h6 className="fw-bold text-dark mb-0">Credit Ledger — Full Statement</h6>
                            </div>
                            {statementLoading ? (
                                ...
                                <div className="table-responsive">
                                    <table className="table table-hover align-middle mb-0">
                                        <thead className="bg-light">
                                            <tr>
                                                {['Date', 'Type', 'Amount', 'Running Balance', 'Reference', 'Notes'].map(h => (
                                                    <th key={h} className="py-3 text-uppercase small fw-bold text-muted" style={{ fontSize: '10px' }}>{h}</th>
                                                ))}
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {statement.length === 0 ? (
                                                <tr><td colSpan={6} className="text-center py-5 text-muted small">No credit transactions found</td></tr>
                                            ) : statement.map(tx => (
